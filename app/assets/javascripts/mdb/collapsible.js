    ...
    return this.each(function() {
      ...
      if (settings.accordion === true) {
        if (settings.contentOpen !== null) {
          $($thisEven[settings.contentOpen]).children('.rotate-icon').toggleClass(settings.arrowRclass + ' ' + settings.arrowDclass);
        ...
        $($thisEven).click(function() {
          if ($(this).next().attr('class') === accord) {
            ...
            $(this).children('.rotate-icon').toggleClass(settings.arrowRclass + ' ' + settings.arrowDclass);
          } else {
        ...
        if (settings.contentOpen !== null) {
          if (Array.isArray( settings.contentOpen )) {
            for (var i = 0; i < settings.contentOpen.length; i++) {
              ...
              $($thisEven[index]).children('.rotate-icon').toggleClass(settings.arrowRclass + ' ' + settings.arrowDclass);
              $($thisOdd[index]).show();
            }
          } else {
            $($thisEven[settings.contentOpen]).children('.rotate-icon').toggleClass(settings.arrowRclass + ' ' + settings.arrowDclass);
